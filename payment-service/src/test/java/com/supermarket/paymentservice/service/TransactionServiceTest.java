...
class TransactionServiceTest {
    ...
    @Test
    @DisplayName("VerifyTransaction: Throws IllegalArgumentException for negative amount")
    void verifyTransaction_NegativeAmount_ThrowsIllegalArgumentException() {
        // Arrange
        Transaction tx = new Transaction();
        tx.setRequiredAmount(100.0);
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            transactionService.verifyTransaction(tx, -50.0);
        });
    }
    ...
    @Test
    @DisplayName("PayByCard: Success processes payment")
    void payByCard_ValidInput_ProcessesAndSaves() {
        // Arrange
        double receivedAmount = orderAmount; // Exact amount
        String cardNumber = "1234567812345678";
        String cardHolder = "Test User";

        // Mock proceedTransaction dependencies
        when(billingServiceClient.getOrderByOrderId(orderId)).thenReturn(sampleOrderDto);
        // Mock the initial save in proceedTransaction
        when(transactionRepository.save(any(Transaction.class))).thenAnswer(inv -> {
            Transaction t = inv.getArgument(0);
            // Assign ID only if it's the first save (status Pending)
            if ("Pending".equals(t.getPaymentStatus()) && t.getTransactionId() == 0) {
                t.setTransactionId(transactionId);
            }
            // Return the object passed to save, potentially modified
            return t;
        });

        // Act
        Transaction result = transactionService.payByCard(orderId, receivedAmount, cardNumber, cardHolder);

        // Assert
        assertNotNull(result);
        assertEquals(transactionId, result.getTransactionId()); // Check ID assigned
        assertEquals(PaymentMode.CARD, result.getPaymentMode());
        assertEquals(cardNumber, result.getCardNumber());
        assertEquals(cardHolder, result.getCardHolderName());
        assertEquals("Completed", result.getPaymentStatus()); // Should be verified
        assertEquals(receivedAmount, result.getReceivedAmount());
        assertEquals(0.0, result.getBalanceAmount());
        assertNotNull(result.getTransactionTime());

        verify(billingServiceClient).getOrderByOrderId(orderId);
        // Verify save called twice (once in proceed, once at the end)
        verify(transactionRepository, times(2)).save(transactionCaptor.capture());

        // Check final saved state
        Transaction finalSave = transactionCaptor.getValue();
        assertEquals("Completed", finalSave.getPaymentStatus());
        assertEquals(cardNumber, finalSave.getCardNumber());
    }
    ...
    // --- payByCash Tests ---
    @Test
    @DisplayName("PayByCash: Success processes payment")
    void payByCash_ValidInput_ProcessesAndSaves() {
        // Arrange
        double receivedAmount = orderAmount; // Exact amount
        ...
        // Act
        Transaction result = transactionService.payByCash(orderId, receivedAmount);

        // Assert
        assertNotNull(result);
        assertEquals(transactionId, result.getTransactionId());
        assertEquals(PaymentMode.CASH, result.getPaymentMode());
        ...
        verify(billingServiceClient).getOrderByOrderId(orderId);
        verify(transactionRepository, times(2)).save(transactionCaptor.capture());
        assertEquals("Completed", transactionCaptor.getValue().getPaymentStatus());
    }
